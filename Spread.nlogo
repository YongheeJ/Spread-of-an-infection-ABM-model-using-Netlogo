...
[
  ...
  recovered?              ;; if true, the person recovred from infection (no more infection)
  susceptible?            ;; if true, the person can be infected
  infection-length        ;; length of infection for infected person
  recovery-time           ;; required time to recover the infection
  nb-infected             ;; number of secondary infections at the end of the tick
...
to setup-people
  create-turtles 150                      ;; there are 150 humans initially
  [
    setxy random-xcor random-ycor         ;; humans are randomly moved
    set recovered? false                  ;; these humans are not recovered
    set infected? false                   ;; these humans are not infected
    set susceptible? true                 ;; but these humans can be infected

    set shape "person"                    ;; set the shape as person
    set color white                       ;; set the color as white

    set recovery-time 30                  ;; set the recovery time as 30

    if (random-float 100 < 5)             ;; person has a 5% chance of infection
    [
      set infected? true                  ;; if random-float is less than 5, person infected
      set susceptible? false              ;; if infected, no more susceptible
      set infection-length recovery-time  ;; after recovery-time, they will be recovered
    ]
    assign-color                          ;; assign colors to identify person's state (Healthy,Sick,Recovered)
  ]
end

to assign-color                       ;; assign colors to identify person's state (Healthy,Sick,Recovered)
  if infected?                        ;; if person is infected,
    [ set color red ]                 ;; it is red color
  if recovered?                       ;; if person is recovered,
    [ set color green ]               ;; it is green color
end

to go
  if all? turtles [ not infected? ]   ;; if there is no more infected people,
    [ stop ]                          ;; stop the simulation
  ask turtles
    [ move clear-count ]              ;; person randomly move
  ask turtles with [ infected? ]
    [ infect maybe-recover ]          ;; infected person will be recovered
  ask turtles
    [ assign-color ]                  ;; assign colors to identify human's state (Healthy,Sick,Recovered)
  tick
end

to move                    ;; define the person movement
  rt random-float 200      ;; randomly generate between 0 and 200
  fd 1                     ;; forward 1 step
end
...
to infect                                                    ;; define infect
   let nearby-uninfected (turtles-on neighbors)              ;; infection can occur to any susceptible person nearby
     with [ not infected? and not recovered? ]               ;; if person is not infected and not recovered

     if nearby-uninfected != nobody                          ;; if there is uninfected person nearby
     [ ask nearby-uninfected
       [ if random-float 100 < virus-blocking-grade-by-mask  ;; that person will be infected by blocking rate
         [ set infected? true
...
to maybe-recover
  ...
  if infection-length > recovery-time              ;; if people have been infected for more than the recovery-time
  [
    if random-float 100 < 80                       ;; then there is a random chance for recovery
    [ set infected? false
      set recovered? true
...
"" ""
PENS
"Infection Rate" 1.0 0 -2674135 true "" "plot (((count turtles with [ recovered? ] + count turtles with [ infected? ]) / 150) * 100)"
"Recovery Rate" 1.0 0 -10899396 true "" "plot ((count turtles with [ recovered? ] / 150) * 100)"
